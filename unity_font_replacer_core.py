...
def load_texture_metadata_sidecar(path: str) -> JsonDict | None:
...
def load_font_bundle_targets_from_game_catalog(
    game_path: str,
    lang: Language = "ko",
) -> tuple[set[str] | None, JsonDict | None]:
...
def scan_fonts(
    ...
    bundle_targets: set[str] | None = None
    catalog_summary: JsonDict | None = None
    if not target_files:
...
def _load_font_assets_cached(
    ...
    sdf_texture_meta = None
    for name_candidate in name_candidates:
        for asset_root in asset_roots:
            texture_meta_path = os.path.join(
                asset_root, f"{name_candidate} Atlas.texture-meta.json"
            )
            sdf_texture_meta = load_texture_metadata_sidecar(texture_meta_path)
            if sdf_texture_meta is not None:
                break
        if sdf_texture_meta is not None:
            break
    ...
    return {
        ...
        "sdf_texture_meta": sdf_texture_meta,
...
def load_font_assets(
    ...
    return {
        ...
        "sdf_texture_meta": copy.deepcopy(cached_assets.get("sdf_texture_meta")),
            ...
            if isinstance(texture_plan, dict):
                ...
                if not applied_raw_alpha8:
                    ...
                    if ps4_swizzle:
                        ...
                        applied_ps4_swizzle, ps4_reason = _apply_ps4_bc_swizzle_to_texture(
                            parse_dict,
                            width=tex_w,
                            height=tex_h,
                        )
                        if lang == "ko":
                            if applied_ps4_swizzle:
                                _log_console(
                                    "  PS4 BC swizzle 적용: UnityPy 인코딩 결과를 8x8 Morton block order로 변환했습니다."
                                )
                            else:
                                _log_console(
                                    f"  PS4 swizzle 건너뜀: {ps4_reason}"
                                )
                        else:
                            if applied_ps4_swizzle:
                                _log_console(
                                    "  Applied PS4 BC swizzle: converted UnityPy-encoded data to 8x8 Morton block order."
                                )
                            else:
                                _log_console(
                                    f"  Skipped PS4 swizzle: {ps4_reason}"
                                )
...
예시:
  ...
  %(prog)s --gamepath "C:/path/to/Image0/Media" --parse
  ...
  %(prog)s --gamepath "C:/path/to/game" --list font_map.json
        """
        gamepath_help = "게임 루트 경로 (PC 예: C:/path/to/game, PS4 예: C:/path/to/Image0/Media)"
        ...
        ps4_swizzle_help = "PS4 BC swizzle 실험 모드 (8x8 Morton block order, mip1 only)"
...
Examples:
  ...
  %(prog)s --gamepath "C:/path/to/game" --list font_map.json
        """
        gamepath_help = "Game root path (PC: C:/path/to/game, PS4: C:/path/to/Image0/Media)"
        ...
        ps4_swizzle_help = "Experimental PS4 BC swizzle mode (8x8 Morton block order, mip1 only)"
    ...
    if args.ps4_swizzle and args.ps5_swizzle:
        parser.error("--ps4-swizzle and --ps5-swizzle cannot be used together")
    ...
    elif args.ps4_swizzle:
        if is_ko:
            _log_console(
                "PS4 swizzle 실험 모드: Console-Swizzler/GFD-Studio 계열 8x8 Morton BC swizzle을 사용합니다. "
                "현재는 BC 포맷 + mip1 only 조건에서만 적용하며, 조건이 맞지 않으면 자동으로 건너뜁니다."
            )
        else:
            _log_console(
                "Experimental PS4 swizzle mode: using the Console-Swizzler/GFD-Studio style 8x8 Morton BC swizzle. "
                "It only applies to BC formats with mip1-only targets and skips unsupported cases automatically."
            )
    else:
        if is_ko:
